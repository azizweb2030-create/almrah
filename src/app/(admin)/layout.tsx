...
const NAV = [{href:'/admin',icon:'📊',label:'لوحة التحكم'},{href:'/admin/users',icon:'👥',label:'المستخدمون'},{href:'/admin/tickets',icon:'🎫',label:'التذاكر'},{href:'/admin/stats',icon:'📈',label:'الإحصائيات'},{href:'/admin/settings',icon:'⚙️',label:'الإعدادات'}]
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <aside className="hidden lg:flex fixed top-0 right-0 bottom-0 w-52 flex-col bg-gray-900 z-30 pt-14">
        <nav className="flex-1 py-3 px-2">
          {NAV.map(item=>(
            <Link key={item.href} href={item.href} className={cn('flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium mb-0.5 transition-colors',pathname===item.href||pathname.startsWith(item.href+'/')?'bg-white/10 text-white':'text-gray-400 hover:bg-white/5 hover:text-gray-200')}>
        ...
        <div className="lg:hidden fixed inset-0 z-50 flex">
          ...
          <div className="relative w-64 bg-gray-900 h-full">
            ...
            <nav className="py-2 px-2">
              {NAV.map(item=>(
                <Link key={item.href} href={item.href} onClick={()=>setOpen(false)} className="flex items-center gap-3 px-3 py-3 rounded-xl text-sm text-gray-400 hover:bg-white/5 hover:text-white mb-0.5">
                  <span>{item.icon}</span><span>{item.label}</span>
                </Link>
              ))}
            </nav>
