...
export default function BirthsPage() {
  ...
  const filtered = births.filter(b => !search || b.mom_id?.includes(search) || (b.babies||[]).some((baby:any) => baby.baby_id?.includes(search)))
    ...
    <div className="space-y-4">
      ...
      {filtered.length === 0 ? (
        <div className="card text-center py-16">
          ...
          <Link href="/births/new" className="btn-primary mt-4 inline-block">سجّل أول ولادة</Link>
        </div>
      ) : (
                ...
                <div className="card-hover">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="text-xs text-gray-500">{b.birth_date} · {alive} مولود حي من {(b.babies||[]).length}</p>
                    </div>
